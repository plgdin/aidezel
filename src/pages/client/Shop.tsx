...
const Shop = () => {
  ...
  const { availableSubcats, availableBrands, availableSpecs } = useMemo(() => {
    ...
    return { availableSubcats: subcats.sort(), availableBrands: brands.sort(), availableSpecs: specsArray };
    ...
    return allProducts.filter((product: any) => {
      ...
      const matchesSpecs = Object.entries(selectedSpecs).every(([filterKey, selectedValues]) => {
        ...
        if (Array.isArray(product.options)) {
            ...
            if (variantGroup && Array.isArray(variantGroup.values)) productValues = variantGroup.values.map((v: string) => standardizeValue(v));
        }
        if (productValues.length === 0 && product.specs) {
            const specKey = findMatchingKey(product.specs, filterKey);
            if (specKey && product.specs[specKey]) productValues = [standardizeValue(String(product.specs[specKey]))];
        }
      ...
      if (searchTerm) {
        ...
        const matchesSub = product.subcategory?.toLowerCase().includes(lowerTerm);
  ...
  const toggleSpec = (key: string, value: string) => {
    setSelectedSpecs((prev) => {
      ...
      if (newValues.length === 0) { const { [key]: _, ...rest } = prev; return rest; }
  ...
  const renderVisualSubcategories = () => {
    ...
    // 2. NEW LOGIC: Hide cards if a subcategory is ALREADY selected (User "entered" one)
    ...
    return (
      <div className="mb-10 animate-in slide-in-from-top-4 duration-500">
        <h2 className="font-bold text-lg text-gray-900 mb-4">Explore {selectedCategory}</h2>
    ...
    <div className="container mx-auto px-4 pt-0 lg:pt-8 pb-24 min-h-screen">
      ...
      <div className="flex flex-col lg:flex-row gap-12">
        <aside className="hidden lg:block w-64 space-y-8 h-fit sticky top-24 flex-shrink-0 animate-in fade-in-0 slide-in-from-left-4 duration-500">
          <div className="flex items-center justify-between mb-2">
            ...
            {(selectedCategory !== 'All' || selectedSubcats.length > 0 || selectedBrands.length > 0) && (
        ...
        <div className="flex-1">
          ...
          {/* 1. VISUAL SUBCATEGORIES (Only shows if NO subcat is selected) */}
      ...
      {showMobileFilters && (
        <div className="fixed inset-0 z-[100] lg:hidden">
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-3xl p-6 pb-safe h-[85vh] overflow-y-auto flex flex-col">
            ...
            <div className="sticky bottom-0 bg-white pt-4 mt-4 border-t border-gray-100 flex gap-3">
              ...
              <button onClick={() => setShowMobileFilters(false)} className="flex-[2] bg-gray-900 text-white py-4 rounded-xl font-bold text-lg shadow-lg">View Items</button>
            </div>
          </div>
        </div>
      )}
